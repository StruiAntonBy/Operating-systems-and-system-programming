...
wstring GetResult(wstring str) {
    ...
    for (int i = 0; i < str.length(); i++) {
        if (str[i] == L'0' || str[i] == L'1') {
            ...
            wstring(*getDecNumber)(wstring);
            HMODULE h = LoadLibrary(L"Dll3.dll");
            if (h==NULL) { MessageBox(NULL, L"Error loading name form library", L"Error loading name form library", MB_OK); }
            getDecNumber = (wstring(*)(wstring))GetProcAddress(h, "getDecNumber");
            if (!getDecNumber) { MessageBox(NULL, L"Error loading function from library", L"Error loading function from library", MB_OK); }
            wstring dec = getDecNumber(sub);
            FreeLibrary(h);
...
void BtnClickEqually(HWND hWnd) {
    ...
    wstring(*getBinNumber)(int);
    HMODULE h = LoadLibrary(L"Dll3.dll");
    if (h == NULL) { MessageBox(NULL, L"Error loading name form library", L"Error loading name form library", MB_OK); }
    getBinNumber = (wstring(*)(int))GetProcAddress(h, "getBinNumber");
    if (!getBinNumber) { MessageBox(NULL, L"Error loading function from library", L"Error loading function from library", MB_OK); }
    SetWindowTextW(hEdit, getBinNumber(Expression(GetResult(wstring(buf)))).c_str());
    FreeLibrary(h);
    delete[] buf;
}
...
INT CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR lpCmdLine, INT nCmdShow) {
    ...
    */
    ...
    name = (wchar_t*)GetProcAddress(h, "Name");
    if (!name) { MessageBox(NULL, L"Error loading name form library", L"Error loading name form library", MB_OK); }
    ...
    wcl.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON1));//Дескриптор пиктограммы приложения.Возможные значения для стандартных пиктограмм : IDI_APPLICATION, IDI_WARNING, IDI_INFORMATION, IDI_ERROR, IDI_QUESTION
